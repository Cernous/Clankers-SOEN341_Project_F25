...
function EventsPage() {
  ...
  const [freeTicketEvent, setFreeTicketEvent] = React.useState<SimpleEvent | null>(null);
  ...
  const handleLucky = () => {
    ...
    if (!freeTicketEvent && Math.random() < 0.10) {
      ...
      <header className="mb-6">
        ...
        {freeTicketEvent && (
          <div className="mb-4 rounded-md border border-green-300 bg-green-50 p-4">
            <p className="font-semibold text-green-700">🎉 You've unlocked a free ticket for: {freeTicketEvent.title}</p>
