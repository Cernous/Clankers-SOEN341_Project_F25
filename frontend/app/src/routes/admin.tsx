...
function AdminDashboard() {
  ...
  async function handleDeleteUser(uid: string) {
    ...
    try {
      await ToolsService.deleteUser({ userId: uid } as any)
      // optimistic remove
                      ...
                      <tr key={u.id} className={i > 0 ? 'border-t' : ''}>
                        ...
                        <td className="px-4 py-2">
                          <button
                            ...
                            className="rounded-md bg-red-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50 cursor-pointer disabled:cursor-not-allowed"
                            title={
