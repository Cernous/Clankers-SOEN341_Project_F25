...
@router.post("/events")
def create_event(
    data: EventAdminCreate, session: SessionDep, user: User = Depends(get_current_user)
):
...
def read_public_event(event_id: int, session: SessionDep):
    return EventPublicRead.model_validate(
        crud.get_event_by_id(session=session, event_id=event_id)
    )
...
def update_event(
    ...
    if user.role == "student":
        raise HTTPException(
            status_code=403, detail="Students can't update events silly"
        )
...
def delete_event(
    ...
    if user.role == "student":
        raise HTTPException(
            status_code=403, detail="Students can't delete events silly"
        )
    if user.role == "organizer" and get_event.organizer_id != user.id:
...
def add_ticket(
    event_id: int, session: SessionDep, ticket: str, current_user: CurrentUser
):
    ...
    if crud.assign_ticket_to_user(
        user_id=current_user.id, event_id=event_id, new_ticket=ticket, session=session
    ):
...
def remove_ticket(event_id: int, session: SessionDep, current_user: CurrentUser):
    ...
    if not crud.remove_ticket(
        user_id=current_user.id, event_id=event_id, session=session
    ):
