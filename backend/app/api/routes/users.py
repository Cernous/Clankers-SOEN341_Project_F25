...
@router.post("/signup/{user_role}")
def create_user(user_role: str, user: UserRegister, session: SessionDep) -> Token:
    """
    ...
    if user_role.lower() not in ["student", "organizer"]:
        raise HTTPException(
            status_code=400, detail="Role not specified or does not exist"
        )
    user_in = UserCreate(
        email=user.email,
...
def get_user(user_id: str, session: SessionDep):
    ...
    # if user.role == user_role.ADMIN or user == current_user:
    return {
...
def update_password(
    passwordform: UserUpdatePassword, session: SessionDep, user: CurrentUser
):
    """
    ...
    if passwordform.new_password == passwordform.current_password:
        raise HTTPException(
            400, "Your new password should not be the same as your current password"
        )
