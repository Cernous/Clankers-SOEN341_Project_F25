...
def get_all_users(session: SessionDep, user: User = Depends(get_current_user)):
    ...
    return session.exec(users_table).all()
...
def delete_user(
    user_id: str, session: SessionDep, user: User = Depends(get_current_user)
):
    """
    ...
    if user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Invalid role used")
    users_table = select(User)
    user = session.exec(users_table.filter(User.id == user_id)).first()
    if not user:
...
def get_num_organizers(session: SessionDep, current_user: CurrentUser):
    ...
    result = len(session.exec(user_table.where(UserRole.ORGANIZER == User.role)).all())
    return {"number": result}
...
def get_average_age(session: SessionDep, user: User = Depends(get_current_user)):
    ...
    if user.role != UserRole.ADMIN:
        raise HTTPException(status_code=403, detail="Invalid role used")
    today = date.today()
    statement = select(User).where(User.date_of_birth.isnot(None))
    users = session.exec(statement).all()
    if not users:
        return {"average_age": None}
    avg_age = sum((today - u.date_of_birth).days / 365.25 for u in users) / len(users)
    return {"average_age": round(avg_age, 2)}


@router.get("/analytics/average_age/{event_id}", tags=["Analytics", "events"])
def get_event_average_age(
    event_id: int, session: SessionDep, user: User = Depends(get_current_user)
):
    """
